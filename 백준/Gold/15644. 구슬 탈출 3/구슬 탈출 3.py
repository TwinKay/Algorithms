...
def back_track(time,r_x,r_y,b_x,b_y,before_k,path):
    global res_min,res_path
    ...
    for k in range(4):# 상하좌우
        ...
        back_track(time+1,r_dx,r_dy,b_dx,b_dy,k,path+[k])

def direct_to_char(directs):
    lst = []
    for d in directs:
        if d == 0:
            lst.append('U')
...
if res_min == INF:
    ...
    print(res_min)
    res_path = direct_to_char(res_path)
    print("".join(res_path))